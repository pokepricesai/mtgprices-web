// scripts/phase3c-build-v2.ts
// Retest Build with the smaller-schema prompt via runAi (generateText loop).
...
async function main() {
  console.log('=== Phase 3C: Build v2 (small schema, generateText + tools) ===\n')
  ...
  const brief = 'Simple mono-blue Commander deck around this commander. Include counterspells, card draw, mana rocks. Basic Islands are auto-filled by the app.'
  const t0 = Date.now()
  const r = await runAi({
    tier: 'reasoning',
    system: BUILD_SYSTEM,
  ...
  console.log(`extractJson: ${json ? 'ok' : 'null'}`)
  console.log(`grounding.ok=${verified?.ok}  grounding.error=${(verified as any)?.error ?? 'none'}`)
  if (!verified?.ok) {
    console.log(`raw head (400ch): ${String(r.text).slice(0, 400)}`)
    console.log(`raw tail (300ch): ${String(r.text).slice(-300)}`)
  }
  if (verified?.ok) {
    ...
    console.log('\nfirst 12 main entries:')
    const ids = verified.value.main.slice(0, 12).map((m: any) => m.oracle_card_id)
    ...
    for (const m of verified.value.main.slice(0, 12)) {
